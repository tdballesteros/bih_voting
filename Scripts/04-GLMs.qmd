...
This script runs linear models modelling nationalist and non-nationalist portions of votes by post-war municipality.
...
cor_matrix <- cor(
  df %>%
    dplyr::select(
      `Proportion of Votes for Non-Nationalist Parties, 1997`, `Deaths per Population`,
      `Minefield Percentage of Municipalty`, `Ethnic Fractionalization, 1991`,
...
model_a1 <- glm(`Proportion of Votes for Non-Nationalist Parties, 1997` ~
                  ...
                  `Turnout Rate, 1997` +
                  `Yugoslav Population Percentage, 1991` +
                  `Other Population Percentage, 1991` +
                  `Absolute Change in Bosniaks, Croats, and Serbs, 1991 to 2013` +
                  `Distance to Croatia, Post-War` +
                  `Distance to Yugoslavia, Post-War` +
                  `Distance to IEBL, Post-War` +
                  `Population Density, 1991 Redistributed` +
                  # `Population Density, 1991 Redistributed Logged` +
                  `Population, 1991 Redistributed Logged` +
                  # `Population, 1991 Redistributed` +
